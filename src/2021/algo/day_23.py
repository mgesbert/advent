...
def part_1(input_data):
    TARGET = {"A": ["A", "A"], "B": ["B", "B"], "C": ["C", "C"], "D": ["D", "D"]}
    ...
    def possible_moves(hallway: list[str], pods: dict[str, list[str]]):
        for pos, amphipod in enumerate(hallway):
            ...
            if all(
                c == "."
                for c in hallway[
                    min(pos + 1, POD_POS[amphipod]) : max(pos - 1, POD_POS[amphipod])
                    + 1
                ]
                ...
                yield (
                    ...
                    (abs(POD_POS[amphipod] - pos) + 2 - len(pods[amphipod]))
                    * COST[amphipod],
                )
                return
        for pod_name, pod in pods.items():
            ...
            for rest_pos in HALLWAY_REST_POS:
                if any(
                    c != "."
                    for c in hallway[
                        min(rest_pos, POD_POS[pod_name]) : max(
                            rest_pos, POD_POS[pod_name]
                        )
                        + 1
                    ]
                ):
                ...
                yield (
                    ...
                    {**pods, pod_name: pod[1:]},
                    (abs(POD_POS[pod_name] - rest_pos) + 3 - len(pod)) * COST[amphipod],
                )
